...
function PRN_Time=SUNBern(SP3_XYZ)
%% Compute Sun position              ***(KamPar)***
Leapsec = 16;
MJD    = SP3_XYZ.data(:,SP3_XYZ.col.JD); %% epo: Y M D h m s
Xctsun  = zeros(length(MJD),3);
Rsun    = zeros(length(MJD),1);
for i=1:length(MJD)
    [X,xctsun,R,L,B] = SUN_bern(MJD(i) - (Leapsec - (350.7512/1000) )/86400);
    Xctsun(i,:) = xctsun';
    Rsun(i)  = R;
end
xctsat = SP3_XYZ.data(:,SP3_XYZ.col.X:SP3_XYZ.col.Z);
rsat = sqrt(sum(xctsat.^2,2))*ones(1,3);
xctsat = xctsat./rsat;   %norm(xctsat)=1
cosphi = dot(xctsat,Xctsun,2);
D = rsat(:,1).*sqrt( 1 - cosphi.^2 );
conditions = ((cosphi<0)+(D<6378137.0));
Find = find(conditions==2);
PRN_Time = SP3_XYZ.data(Find,[1,13]) %PRN and Time
end
